...
findings are deferred as a coordinated re-sweep task (see the debt register and
the Phase 5 finding below).

### Phase 5 finding

The calibration is too marginal to absorb even correct mechanics fixes.
...
The correct remedy is a coordinated fix plus a full `npm run sweep` re-center,
...
Landed via one coordinated set plus the margin re-sweep (20 iters, 12-knob
diff): D4 (FT-phase fatigue) and D5 (relocUntil hygiene) are in, and the band
gate is back to 16/17 at n=48 (pace 94.4→in-band, FT% 80.9→in-band;
assisted-share remains the structural D1 miss). Fidelity identities all pass;
Vance leads his team in assists again.
...
| W28 | Probe×coupling interaction: measured, destructive, cohort-contingent; the gate that deferred W19's flip. Same seeds, assembly A/B (b2-trial-setB vs -setA/-setC): adding the probe (0.15/0.08) to the live coupling costs θ 0.098 → 0.038 on the std cohort and doubles the fixed-pool talent-drift tax (keep 0.91 → 0.26-0.28; the two largest real drifts drop to 0.20/0.26 of baseline, one flips sign), while the margin effect runs opposite directions by cohort (std fam-a \|m\| +1.6 anti-reversion; fixed pools \|m\| −1.7 over-compression to 11.2 with close-share 30%). Mechanism hypothesis on file: the probe's early-shot suppression blocks the early-offense channel that talent (and the coupling) expresses through. What the probe buys everywhere: +0.11-0.12 passes/poss at in-band texture. Successor-arc requirement: price the interaction, not the standalone dose alone; land probe and coupling in separate calibrations. | b2-trial-setB/-setA/-setC findings; b2-verify-std-a §cause-isolation | recorded (measured); gates the W19 successor arc |
...
(timeouts/replay/team rebounds read as markers of real text). Protocol notes:
unicode pseudonymizer bug (fixed), renderer lacks bbref's "at rim" variant.
Full provenance in data/nba/flow-reference.json.
