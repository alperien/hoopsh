...
async function main(): Promise<void> {
  ...
  while (league.season < startSeason + SEASONS) {
    ...
    if (digest.seasonRolledTo !== undefined && seasonAtTick < digest.seasonRolledTo) {
      ...
      reports.push({
        ...
        taxTeams: Object.keys(league.teams).filter(t => capSheet(league, t, seasonAtTick).overTax).length,
        capViolations,
  ...
  // ---------- verdicts ----------
  console.log('\n================ the report ================');
  const bands: Array<{ name: string; check: (r: SeasonReport) => boolean; note: string }> = [
    { name: 'league scoring 105-125/g', check: r => r.ptsPerGame >= 105 && r.ptsPerGame <= 125, note: 'research 05 A' },
    { name: 'pace 94-104', check: r => r.pace >= 94 && r.pace <= 104, note: 'research 05 A' },
    { name: '3PA 30-42/g', check: r => r.tpaPerGame >= 30 && r.tpaPerGame <= 42, note: 'research 05 A' },
    { name: 'top scorer 26-37 ppg', check: r => r.topScorerPpg >= 26 && r.topScorerPpg <= 37, note: 'research 05 C leaderboards' },
    { name: 'minutes leader 33-38.5', check: r => r.minutesLeader >= 33 && r.minutesLeader <= 38.5, note: 'the first thing stat heads check' },
    { name: 'wins sd 9-16', check: r => r.winsSd >= 9 && r.winsSd <= 16, note: 'real ~12.9' },
    { name: 'best team 55-73 wins', check: r => r.topWins >= 55 && r.topWins <= 73, note: 'a 60-win team most years' },
    { name: 'worst team 9-28 wins', check: r => r.bottomWins >= 9 && r.bottomWins <= 28, note: 'bad, not 5-77' },
    { name: 'mean age 24.5-28', check: r => r.meanAge >= 24.5 && r.meanAge <= 28, note: 'league ~26.4' },
    { name: 'tax teams 2-12', check: r => r.taxTeams >= 2 && r.taxTeams <= 12, note: 'economy alive at both ends' },
    { name: 'no unsigned-star pileup', check: r => r.faUnsignedStars <= 6, note: 'September market clears' },
  ];
  for (const band of bands) {
    const misses = reports.filter(r => !band.check(r));
    console.log(`${misses.length === 0 ? 'PASS' : 'MISS'}  ${band.name}  (${band.note})` +
      (misses.length ? `  missed in: ${misses.map(m => m.season).join(', ')}` : ''));
  }
