 ...
 * (rims[0]) in the second half. That flip at intermission is real basketball
 * (teams swap baskets at halftime; the court itself never moves).
 */
 ...
 * the rule reshapes the line to keep it on the court. That's why corner
 * threes (22 ft) are shorter than above-the-break threes (23.75 ft): a side
 * effect of the court's fixed width, not a design choice.
 *
 ...
 *    so the primary ball-handler starts as a three-point threat).
 ...
 *    lane lines: a classic pass-and-cut or pick-and-pop landing spot, and
 *    the canonical mid-range real estate. A catch here is the 16-footer.
...
export function spacingSpots(court: Court, rim: V2): { key: string; pos: V2 }[] {
  ...
  return [
    ...
    // creator's assists inflate to 12-14/game, which amplifies the
    // pre-existing structural assist-economy overshoot (D1). D3 is therefore
    // coupled to D1: land the assist-model fix first, then this becomes
    // safe. Until then the junk-2 trickle is the lesser distortion.
    { key: 'corner_l', pos: { x: baselineX + dir * 4, y: cy - 21.5 } },
    { key: 'corner_r', pos: { x: baselineX + dir * 4, y: cy + 21.5 } },
    { key: 'dunker', pos: spot(4, cy + 9) },
    // low blocks: post-up spots, ~first hash beside the key
