 ...
 * result is the byte-identical pre-layer engine.
 *
 ...
 *
 * The activation follows the coaching calculus, not a script:
 ...
 *    24 per possession (the "scaled by deficit and remaining
 *    possessions" window);
 ...
 * phase is set (the one choke point every stoppage routes through,
 * so a timeout can only ever happen where real ones do). Only the team
 * about to inbound may call one (the possession requirement). Two
 ...
 *
 *  2. stop_run: the opponent has timeoutRunPts unanswered. Call time,
 ...
 *     ~45 s, just when a collapsing lead most wants the whistle).
 *
