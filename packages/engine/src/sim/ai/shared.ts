...
/**
 * Creation score, the single usage-hierarchy definition. Used by both ball
 * routing (decideBall's re-initiation pull) and action initiation
 * (actionTick's rank gate); one definition so "who should run the offense"
 * never disagrees between deciding to pass and deciding to call a screen.
 */
...
/**
 * The pull-up half of the mid green light: a joint gate over the mid
 * appetite and the off-dribble appetite, as a geometric mean. Both are
 * required; either at/below its floor vetoes the light entirely (a post big
 * with no dribble game never snakes, a pull-up three hunter with no mid
 * appetite never stops short). But multiplying two sub-1 gates would
 * double-count moderation (a 44-shotMid/68-pullUp microwave scorer fell to
 * a 0.33 light and never fired; the mean keeps him at 0.57). One definition
 * read by the mid-range decisiveness term (concepts.ts, taking the pull-up)
 * and the drive stop-short (game.ts, attacking to the pull-up spot), so the
 * player who snakes to the elbow and the player who rises once there are
...
export function moveSpeed(s: GameState, a: Agent): number {
  ...
  if (a.intent === 'defend') {
    ...
    // speed, so quick-footed guards contain drives better than fast
  ...
  // is held rather than chased, and the ball-holder keeps the faster cruise.
