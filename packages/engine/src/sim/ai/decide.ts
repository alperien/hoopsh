...
export function decideBall(s: GameState): BallAction {
  ...
  // urgency window (any shot beats a violation). This single curve produces
  // patient early-clock offense and desperate late-clock heaves.
  ...
  if (myShot.zone === 'three') {
    ...
    // green light to shooters and leaves hubs and rim-runners alone.
    // Applied flat, the texture re-tune's appetite=1.3 dragged the
  ...
  // Usage pressure: the closed loop that ties load to identity. The dial
  ...
  // off. EV alone can't express this: a 99-vision hub's passes always
  // out-value his shots, yet the real player takes 17 a game because
  ...
  if (!driving && distToRim > A.driveMinDistFt && s.poss.phase !== 'advance') {
    ...
    // (nobody helped; its value is already in the high finish EV), and a
    // crowded lane is where the kick lives. Ungated, every drive carried a
    // phantom kick premium and the paint flooded league-wide (rim share 86%,
    // 3PA 13%). Without the option term entirely, the rim attempt alone
    // rarely beats the continuation value and drives never launch (Stage 2
    // probe: 2 drive wins in 657 decisions for the league's best handler).
    // A stalled drive still just resets to the continuation value.
    ...
    // bar correctly rejects it. This routes drive volume, and the assists it
    // creates, through the creation hierarchy without a single special case.
 ...
 * direct utility penalty, so a packed paint deters drives and (via the
 * kickout branch) help defense produces open shooters.
 *
