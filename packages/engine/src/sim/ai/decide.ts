...
export function decideBall(s: GameState): BallAction {
  ...
  if (!driving && distToRim > A.driveMinDistFt && s.poss.phase !== 'advance') {
    ...
    // TRAP (a7 line audit F1): the no-on-ball fallbacks below (gap 8,
    // contain 50) are behavioral constants, and the gap fallback sits BELOW
    // onBallDefender's 12 ft cutoff — so a defender stepping from 11.9 ft
    // to 12.1 ft DROPS the computed P(get downhill) (non-monotone at the
    // boundary; ~4% of drive evaluations hit the null branch, measured).
    // The error direction understates the most unguarded handlers. Fixing
    // it means re-pricing drive volume (band-gated) — registered for the
    ...
    // gap above — see the F1 note there)
