 ...
 * serializable object. Nothing else in the engine may hardcode a constant
 * that affects outcomes: the harness sweeps this file (`npm run sweep`)
 * against real NBA acceptance bands, and a number hidden elsewhere is a
 * number the optimizer cannot reach.
 *
 * One exception: cosmetic, replay-only positioning. A handful of constants
 * that place bodies for the replay and nothing else stay inline where
 * they're used: the dead-ball freeze spots (possession.ts setupDeadTargets),
 * the free-throw lane alignment (fouls.ts enterFreeThrows), and similar
 * "looks plausible" geometry. No probability, EV, rate, or timing model
 * reads them; they move pixels in the viewer, not outcomes, so they are
 * deliberately kept off the sweep surface (a knob the optimizer can't affect
 * the bands with is noise in the search space). Each such site says so in a
 * comment. An inline number that feeds a make/foul/turnover/rebound/decision
 * path is a bug against this rule; move it here.
 *
 ...
 *    contest": rating influence enters as n(rating) ∈ [-1,+1], which is
 *    exactly 0 at rating 50 (see model/derived.ts).
 *
 ...
 *    compares "expected points if I shoot now" against "expected points if
 *    we keep working" (`continuationMax`), so a bias of 0.1 means "worth a
 *    tenth of a point". That is the scale to think in when nudging these.
 *
 ...
 *        carry odd precision (0.485, 1.449) because a machine chose them;
 *        do not "tidy" them without re-running the sweep.
 *      FEEL: hand-set for plausible motion/timing, not statistically
 *        constrained (mostly the `move`, windup, and dead-ball timings).
 *
 * 5. Coupling. These knobs are not independent. Foul rates feed shot EV
 *    (a foul is worth free throws), which changes shot selection, which
 *    changes pace and shot mix. Historical example: raising `foul.shootRim`
 *    from 0.475 to 0.5 collapsed the league three-point rate from ~38% to
 *    30% because every player suddenly preferred driving. Re-verify with
...
export interface SimParams {
  ...
  shot: {
    ...
    /** shooting-foul probability multiplier when the shot went in (an and-one
     *  is rarer than a foul on a miss) */
  ...
  move: {
    ...
    /** non-sprint crash/boxout repositioning speed (share of max): short and quick, not a dash */
  ...
  /**
   * Endgame layer (concept 6, game-state urgency). Lives under
   ...
   * they define when late-game behavior activates. That is identity-shape,
   * not a rate (see the knobs.ts doctrine header).
   */
  endgame: {
     ...
     *  last-shot patience for the hurry (down 1-3, one possession can tie or
     *  win; down 4+ needs multiple possessions, so waiting costs the game). REAL */
     ...
     *  NBA teams hunt the ~0:28-0:38 release for this arithmetic */
     ...
     *  start the fouling before this. REAL: ~0:35 is where trailing
...
export const defaultParams: SimParams = {
  ...
  shot: {
    ...
    fatigueCoef: -0.35,
    // Free throws are modeled in percentage space, not logits.
    ...
    // Windup = seconds between "decides to shoot" and release. It creates
    // the catch-and-shoot vs closeout race, so a defender who is 8 ft away
    // when the decision is made may arrive in time to contest. Real release
    // times: ~0.4 s for a quick catch-and-shoot, ~0.55 s for a gathered
    // pull-up. REAL-ish/FEEL.
  ...
  foul: {
    ...
    // vs the prior comment's own "deliberately rare" intent; that made it
    // the third-largest whistle category and ~30% of all turnovers (every
  ...
  reb: {
    ...
    // weights are discounted. This is the primary lever on ORB% (band 20-30%). SWEPT.
    offWeightMult: 0.6,
    ...
    // assisted share upward against its band ceiling. Team rebounds remain
    // a normal sight in the log (~7/game plus the FT dead-ball
    // formalities). REAL anchor, FEEL discount.
  ...
  decide: {
    ...
    quickCatchSec: 0.9,
    // The number with the most reach in the engine. Expected points of
    // "keep working this possession" with a full shot clock ≈ 1.45. Every
    // shot decision is a comparison against this: shoot only if the look
    // beats what the possession is otherwise worth. It sets pace, shot
    // selection, and shot quality at once: raise it and teams hunt better
    // shots (slower, more efficient), lower it and they fire early. Real
    // NBA offenses average ~1.10-1.15 points/possession; this sits above
    // that because it represents the value of a possession being continued
    // from a live-ball state, not its average outcome. SWEPT.
    ...
    // (facing the wrong way, cross-matched, floor balance gone), a better
    // break than the push off a defensive rebound (the retreat is already
    // downhill-facing). REAL: transition off steals runs ~1.2-1.3 PPP vs
    // ~1.05-1.1 off rebounds, and the sim's measured gap was here: after a
    // steal the clock-only continuation reset to its maximum (~1.47), the
    // thief's rim look priced at uShoot ≈ -0.41 (P(shoot) 14.8%/decision),
    // and steal->score-in-6s ran 12-17% vs the real 29.3%
    // (flow-reference.json). Sized by 12-game probes on two seed bases:
    // 0.35 lands conversion at 27.9/29.3% with league FGA at the
    ...
    // steal-gated, its headcount scaling faded as defenders got back
    // during the push, gutting per-attempt quality at the finish; a
    ...
    // penetrate until arrival, then finish or spray, the same arrival
    // principle as the phase boundaries. FEEL.
  ...
  move: {
    ...
    // defGapGravityFt against a maximum-gravity shooter. This pair is how
    ...
    // because every small sag adjustment ran at full lateral speed. A defender
    // in his stance shuffles; the 1.15x sprint multiplier still applies to
    ...
    // multiplier, so this only walks off-ball spacing moves). FEEL.
    offBallWalkMult: 0.3,
    // The bring-up: at the 0.72 cruise the handler crossed halfcourt at
    // ~13.7 ft/s (9+ mph) every possession, the largest single contributor
    // to the speed signature (avg live speed 6.55 vs NBA ~4.2, and the
    // friction signature it drives; see INTERNALS and the speed-pin
    ...
    // the arc plus a step, and a defender beyond it cannot influence the
  ...
  sub: {
    ...
    // integrity clean, bands untouched, starters' rest −0.8…−0.9 min/g.
    ...
    // mechanistically rather than being masked.
  ...
  ai: {
    ...
    // shot.contestCoef) directly. This is the live coupling: the channel
    // carries the margin mean-reversion after channel 1 measured null (see
    ...
    // deliberately no urgency fade, since defense manufactures no
    // violations (the asymmetry vs channel 1 is documented at
    ...
    // Spacing is held until the ball bends the defense; then shooters
    // shake. While a drive is live, a shooter drifts away from his
    // defender, restoring the open catch. Without this, stillness strangled
    // the catch-and-shoot economy: 3PA share pinned at ~24% and the sweep
    ...
    // and hit 34.9 PTS on 18 threes (fidelity incident). FEEL.
    ...
    // ceil× reluctance applies; at 1.0 it drops to ceil−1, which still
    // avoids leaving elite shooters but rotates off average gravity much
    // more willingly. FEEL: the range is [ (ceil−1)×helperGravityWeight,
    ...
    // matching the late-clock skew of real mid attempts.
    ...
    // the very coverage that makes the middy available.
    ...
    // never at 16 ft can never shoot from 16 ft, and pre-fix no spacing
    // spot or action ever stationed anyone in the 14-20 ft band):
    ...
    //     score 0) can never pop, since their defender parks in the paint
    //     and an elbow catch would be the defense's win, exactly the shot
    //     the green-light floor already refuses them.
    ...
    pnrPopFeedBonus: 0.3,
    // FEEL. Mid-pop bigs are prized screening partners in real offenses
    ...
    // out of ~6, pass options beating the shot in every near-tie (fidelity
