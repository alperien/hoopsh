...
export interface Attributes {
  ...
  // scoring
  finishing: number; // layups, dunks, rim pressure — resolve.ts zoneSkill for 'rim' and (partially) 'paint'
...
export interface Tendencies {
  ...
  /** appetite for putting the ball on the floor and attacking — ai.ts decideBall uDrive tendency term (drive utility), scaled by the decide.driveAppetite SimParams knob (a params dial, not a team tactic) */
  drive: number;
  ...
  /** willingness to crash the offensive glass instead of getting back on defense — ai.ts onShotReleased crash-vs-getback roll when a teammate's shot goes up */
  crashOffReb: number;
