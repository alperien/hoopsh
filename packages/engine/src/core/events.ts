/**
 * The event stream, hoopsh's main public contract.
 *
 * Every discrete outcome in a game is a typed event. Stats, narration,
 * viewers, and future experiences are all pure consumers of this stream. The
 * engine guarantees the stream fully describes the game (box scores are
...
/**
 * How a shot was created, in basketball terms. Feeds the make-probability
 ...
 * `heave`: a desperation end-of-clock/end-of-period launch from distance,
 ...
 * defense-only in the current model.
 */
 ...
 * no-shot buzzer-beater situation).
 */
 ...
 *   much basketball has happened). Two events during the same dead ball
 *   share the same `t`.
 ...
 *   viewers/replays key on: it lets a replay show free-throw routines and
 *   inbound repositioning as real elapsed time instead of instantaneous
 *   jumps. `wt` is always >= `t`, monotonically non-decreasing across the
 *   event stream, and matches `replay/replay.ts` frame timestamps and
 *   `LineupSnapshot.t` exactly (both key on the same wall clock).
 *
...
/** Marks the start of a new possession for `team`. Pairs 1:1 with a later `possession_end` for the same possession. `kind` says how it starts: 'inbound' is a dead-ball inbound (make/miss-and-FT/OOB/violation aftermath); 'live_rebound' means the defense grabbed a live-ball defensive rebound and plays on without a stoppage; 'steal' means a takeaway (bad pass or reach-in) starts the new team's possession immediately, ball-in-hand; 'tip' is the opening possession of a period off a jump ball. */
 ...
 * never produces a `pass` event. `kind` values: 'normal' is a standard
 * halfcourt pass; 'kickout' is a pass out of a live drive (sim/ai.ts
 * decideBall labels any pass while `driving` a kickout); 'outlet' is a pass
 * during transition phase (fast break ball movement); 'entry' is the feed
 * to a posted big (assigned by sim/ai/decide.ts when the post action is
 * 'posting' and the poster is settled on the block); 'handoff' is a
 * dribble-handoff, the hub screening for the receiver as he hands it off
 * (assigned by sim/ai/decide.ts when the DHO receiver has sprinted into
 * range; the catch stuns his trailing defender, see sim/passing.ts). Entry
 * and handoff are both live AI code paths today.
 */
...
/**
 * A shot attempt, made or missed. Shot charts are built directly from
 * `x`/`y`/`distFt`/`zone`/`three`. Invariants:
 * `points` is always 0 when `made` is false, and always `three ? 3 : 2` when
 * `made` is true; it is never inferred from `zone` alone (zone
 * 'rim'/'paint'/'mid' are all worth 2). `assist` is present only when `made`
 * is true and the passer was still on-court at the moment of the shot
 * (sim/shooting.ts startShot explicitly excludes a passer who was subbed out
 * between his pass and this shot: no assists from the bench); a missed shot
 * never carries an `assist` even if it came right off a pass. `blockedBy`
 * can only be set when `made` is false (sim/shooting.ts startShot only rolls
 * for a block on a would-be miss, so the raw make-percentage calibration
 * stays clean: blocks subtract from misses, not from makes). `foul` is
 * present when shooting-foul contact occurred on this attempt regardless of
 * make/miss: `andOne: true` iff the shot also went in (bonus single free
 * throw); `andOne: false` means the shot missed and `ftAwarded` (2 or 3,
 * matching shot value) free throws follow. `contestedBy`/`contest` describe
 * the closest defender's contest level at release (0 wide open .. 1
 * smothered), independent of whether that defender is also `blockedBy` or
 * the foul's `by`.
 */
 ...
 * total length: the second shot of a two-shot foul is `n: 2, of: 2`; an
 * and-one is always `n: 1, of: 1`. A shooter can appear across multiple
 ...
 *    player line, the official-scoring convention (stats/box.ts).
 ...
 * maybeTimeout, invoked from sim/possession.ts deadBall). `reason` values:
 * 'stop_run' means the opponent has scored `params.endgame.timeoutRunPts`
 * unanswered and the coach stops the bleeding; 'advance' means a trailing
 * team late in the final period burns a timeout so the ensuing inbound
 ...
 * `remaining` is the calling team's budget after this timeout (budget per
 * game: rules.timeoutsPerGame). The game clock never runs during the
 * timeout; `wt` keeps advancing so replays show the huddle as real elapsed
 * time.
 */
 ...
 * one element. The array shape allows a future multi-player swap without a
 * breaking event-schema change; that doesn't happen today. Fires only at
 * dead-ball moments or an immediate fouled-out replacement (never
 * mid-live-play otherwise); see sim/subs.ts checkSubs/replaceFouledOut.
 */
...
/** Every event a game can ever emit, as one union discriminated on `type`. */
