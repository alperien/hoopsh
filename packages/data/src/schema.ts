...
// Set views of ATTR_KEYS/TEND_KEYS for the unknown-key rejection in
// validatePlayer() below (string sets, because the keys under test come from
// a hand-edited pack, not from the engine's own types).
const ATTR_KEY_SET: ReadonlySet<string> = new Set(ATTR_KEYS);
const TEND_KEY_SET: ReadonlySet<string> = new Set(TEND_KEYS);
...
function validatePlayer(p: unknown, path: string, issues: ValidationIssue[]): void {
  ...
  if (!attr) issues.push({ path: `${path}.attr`, message: 'missing attributes' });
    ...
    // Unknown keys inside a rating bag are rejected, not ignored — the engine
    // walks EVERY key of attr/tend (game.ts assertValidRatings) and throws on
    // any non-numeric value, so an annotation like `"note": "traded"` would
    // pass a known-keys-only validator and then crash simulateGame at tip-off
    // (JSON has no comments, so a string note inside attr IS the natural
    // hand-edit). A numeric unknown key is no better: the engine silently
    // ignores it, and a typo'd rating that quietly does nothing is exactly
    // the "plays nothing like what its numbers say" trap this validator
    // exists to prevent. Same stance as the generated JSON Schema's
    // additionalProperties: false (tools/gen-schema.mjs enumerates the
    // remaining validator/schema deltas).
    for (const k of Object.keys(attr)) {
      if (!ATTR_KEY_SET.has(k)) {
        issues.push({ path: `${path}.attr.${k}`, message: `unknown attribute "${k}" — not one of the ${ATTR_KEYS.length} engine attribute keys` });
      }
    }
  }
  ...
  if (!tend) issues.push({ path: `${path}.tend`, message: 'missing tendencies' });
    ...
    // same unknown-key stance as attr above — assertValidRatings walks tend too
    for (const k of Object.keys(tend)) {
      if (!TEND_KEY_SET.has(k)) {
        issues.push({ path: `${path}.tend.${k}`, message: `unknown tendency "${k}" — not one of the ${TEND_KEYS.length} engine tendency keys` });
      }
    }
  }
}
