 ...
 *
 * This file is the mod surface: the one place in the codebase that defines
 * what a hand-edited team file is allowed to look like. Anyone writing a
 * custom roster (or, eventually, the roadmap's visual editor) only has to
 * satisfy validateTeamPack(); they never need to know how the engine
 * internally represents a Team. The boundary runs both ways:
 ...
 *
 * Validation policy: strict rejection, not lenient fill-in. Every problem
 * becomes one ValidationIssue with a JSONPath-style `path` (e.g.
 ...
 * There is no partial-pack recovery and no defaulting a missing rating to
 * 50. A pack either fully satisfies the schema or it's rejected outright
 * with a full list of every problem, not just the first one hit, so a
 * hand-editor gets one error dump to fix everything instead of a slow
 * one-issue-at-a-time loop. The alternative, silently defaulting bad
 * fields, was rejected: a silently-defaulted rating would make a custom
 * roster play nothing like what its numbers say, and a data pack is
 * supposed to be an honest description of a team.
 */
...
// via `Object.keys` on a sample object, so validation is exhaustive even for
// a pack that's missing a key entirely (Object.keys on the pack's own data
// would only ever find what's already there). Keeping ATTR_KEYS/TEND_KEYS in
// sync with @hoopsh/engine's Attributes/Tendencies types is a manual
...
// first) to the shared `issues` array. This is what lets validateTeamPack()
// below hand back a complete error report for a whole broken pack in one
// pass, instead of the hand-editor fixing one field, re-running, hitting
...
function validatePlayer(p: unknown, path: string, issues: ValidationIssue[]): void {
  ...
  // and simulateGame() hard-throws on a non-finite body measurement, so a
  // pack without it validates here only to crash at tip-off. Same
  // crash-prevention rationale as the tactics check below. Finiteness only,
  // no range: the engine itself imposes no weight range even in its strict
  // tier, and the pack validator should not out-strict the engine
  // (implausible-but-finite weights are roster-validate warning territory,
  // not rejection).
 ...
 * (empty array = valid). Early-outs only where a missing field makes deeper
 * checks meaningless (e.g. no `team` object at all means there's nothing to
 * validate players against); otherwise keeps accumulating into the same
 * `issues` array so unrelated problems (a bad formatVersion and three
 * players with out-of-range ratings) all surface together.
 */
export function validateTeamPack(pack: unknown): ValidationIssue[] {
  ...
  // name/abbrev are required by the Team type and displayed by every
  // consumer (box score header, play-by-play score lines, replay metadata).
  // A pack without them doesn't crash; it produces "undefined 98, undefined
  // 111" narration, which breaks the pack-is-an-honest-description rule just
  // as surely as a silently-defaulted rating would.
  ...
  // unconditionally, with no fallback. A pack missing tactics wouldn't fail
  // gracefully at sim time; it would crash mid-game the first time the AI
  // needs a tactics-driven decision. Rejecting it here, at load time with a
  // clear message, beats that runtime crash. The schema is acting as a
  // crash-prevention gate here, not just a style check.
 ...
 * pack ready to JSON.stringify to disk. This is the export-rosters.ts
 * harness script's whole job, and the inverse of loadTeamPack() below. */
