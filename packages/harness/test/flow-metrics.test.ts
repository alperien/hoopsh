 ...
 * flow-metrics.ts's OREB-base comment (player+offensive+non-deadBall only,
 * scan-stop rules), and the reduceFlows share-basis comments (b9-F1 pooled
 * denominator). Streams follow the stats/test/box.test.ts mk() convention:
 * every event stamps both time axes plus period/clock/score (AGENTS §1.5),
 * scores move by real basketball increments, and score is AFTER the event.
 *
...
describe('OREB base, putbacks, second-chance marking (flow-metrics.ts rebound-case comment)', () => {
  it('only player+offensive+non-deadBall rebounds count as OREB or mark a second-chance possession', () => {
    // spec: "The OREB base is PLAYER offensive rebounds only" — team-rebound
    // bookkeeping rows and dead-ball FT formalities are excluded from the
    // denominator AND from second-chance marking (scan finding b4-1/c3-F1)
    const events: GameEvent[] = [
      // possession 1: live player OREB, putback 3s later
      ...
      // possession 2: playerless TEAM offensive rebound — excluded everywhere
      mk({ type: 'possession_start', team: 1, kind: 'inbound' } as Partial<GameEvent> & { type: 'possession_start' }, 19, [2, 0]),
      shot(1, false, 24, [2, 0]),
      ...
      // possession 3: dead-ball FT-formality rebound (playerless, offensive) — excluded
    ...
    const f = gameFlow(events);
    expect(f.oreb).toBe(1);             // the player board only
    expect(f.putback6).toBe(1);         // shot by rebounding team 3s later
    expect(f.secondChancePoss).toBe(1); // possessions 2 and 3 never marked
    expect(f.poss).toBe(3);
    // possession length = possession_end.t - possession_start.t (flow.ts def)
