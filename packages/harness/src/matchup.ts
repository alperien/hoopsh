...
export async function simulateMatchup(
  ...
  // index coverage, not just count — same duplicate/missing-index guard as
  // runSeason (season.ts), same B3-3 rationale: a doubled sim index passes
  // the count check yet skews the whole margin distribution silently
  for (let i = 0; i < outcomes.length; i++) {
    if (outcomes[i]!.index !== i) {
      throw new Error(`simulate seam returned duplicate/missing sim indices (sorted position ${i} holds index ${outcomes[i]!.index})`);
    }
  }
