...
/**
 * Combine two accumulators field-by-field.
 *
 * UNWIRED — no consumer in the repo today (AGENTS.md DO-NOT #5; scan
 * finding B2-3, which also corrected this comment's false provenance: it
 * claimed the parallel batch runner and the sweep used it, but the batch
 * runner reduces per-game summaries via accumulate() in the PARENT (cli.ts)
