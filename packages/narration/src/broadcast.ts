 ...
 * (provider.ts) together into one merged script. The engine never depends
 * on this file; it only consumes `GameEvent`s already produced by a
 * finished sim (AGENTS.md §1.3/§6).
 */
...
export async function buildBroadcastScript(
  ...
  // Windowing: raw events accumulate into `buffer` and a window closes
  // (flushes to the provider) at the first of three boundaries:
  ...
  // Each flushed window is a fresh, independent call to
  // `provider.generate()` — the caller side of CommentaryProvider's
  // stateless-per-window design documented in provider.ts.
  ...
  // written) that extracts storyline notes from a provider's output. Wire
  // it when an LLM provider starts carrying narratives across windows
  // (narration is frozen, so this waits with it).
  ...
  // flush the trailing buffer: game_end always lands here (period_end
  // flushes right before it), and providers get to react to the final buzzer
  if (buffer.length > 0) {
  ...
  // (false→0) before 'color' (true→1). At a shared timestamp the play-by-
