...
export class ContextTracker {
  ...
  update(e: GameEvent): NarrativeMoment[] {
    ...
    // unconditionally on every event (not just `scored` ones, unlike runs/
    // milestones/lead-changes above) because clock ticking into the window
    // can make this true even on an event that isn't itself a score (e.g. a
    // rebound at 4:59 doesn't qualify, but the very next non-scoring event
    // after the clock crosses 3:00 should still catch it rather than waiting
    // for the next basket).
