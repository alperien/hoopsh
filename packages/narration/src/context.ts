 ...
 * on hoopsh's events looks like. The engine never depends on this package
 * or anything it decides; narration purely reads `GameEvent`s and never
 ...
 *    reaches the final period with the game clock at or under 3:00 and the
 *    margin at or under 6 points. All three conditions are basketball's
 *    working definition of "winning time": a blowout in the final minutes
 *    isn't clutch, and a one-point game in the first quarter isn't either.
 *
...
export class ContextTracker {
  ...
  // taxonomy note above). This latch is what makes it "once", not the
  // threshold check itself, which would otherwise re-fire on every scoring
  // play deep in a close final period.
  ...
  update(e: GameEvent): NarrativeMoment[] {
    ...
    if (scored) {
      ...
      // player milestones: the exact-vs-threshold subtlety doesn't apply
      // here because the check is `pts >= bar` (not `===`), so a player
      // jumping from 19 to 22 on a three still crosses the 20 bar
      // correctly. The `lastMs < bar` guard (not the crossing arithmetic)
      // is what prevents re-firing, and it tolerates skipped exact values.
      ...
      // i.e. an actual prior leader, to suppress a false "tie"/"lead_change"
      // moment on the very first score of the game (0-0 -> 2-0 is team 0
      // taking an unremarkable first lead, not a "lead change" from
      // anything).
      const [h, a] = e.score;
      ...
      // leaves lastLeader pointing at whoever led just before the tie, so
      // if that same team retakes the lead next basket it's correctly not
      // flagged as a lead_change: they never lost "leader" status in this
      // tracker's bookkeeping, only the scoreboard did.
      if (leader !== -1) this.lastLeader = leader;
    }

    // clutch time: final period, last 3 minutes, margin within 6. Checked
    // unconditionally on every event, not just `scored` ones like runs/
    // milestones/lead-changes above, because clock ticking into the window
    // can make this true even on an event that isn't itself a score. A
    // rebound at 4:59 doesn't qualify, but the very next non-scoring event
    // after the clock crosses 3:00 should still catch it rather than
