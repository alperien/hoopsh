...
// harden the bake: (1) escape EVERY "<" inside the JSON as the unicode
// escape backslash-u003c (valid JSON, round-trips to '<') — the earlier
// "</"-only escape stopped a name containing </script> from closing the
// tag, but "<!--" followed by "<script" put the HTML tokenizer into the
// script-data-double-escaped state, where the template's real </script> is
// consumed as text and the whole baked page dies blank (not XSS — nothing
// executes — but a hostile replay corrupted the standalone file). In valid
// JSON a "<" can only occur inside a string, so the blanket escape is
// complete and semantics-preserving. (2) splice via slice instead of
// String.replace, whose $-patterns corrupt replacements containing bare
// dollar signs (e.g. a team named "Team $1")
