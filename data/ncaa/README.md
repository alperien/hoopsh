...
| `teamFoulBonusAt` | 7 | Bonus does start at the 7th team foul per half… | see finding R1 |
...
have used historically") is factually wrong for NCAA men: one-and-one is the
current rule, not a historical one. Modeling impact at FT% ≈ 71%: a
...
(NCAA women eliminated the one-and-one: 2 shots at the 5th foul of each
quarter. The flat rule the engine implements is the women's shape, not the
men's.)
...
incorrectly start with a clean foul slate, and OTs are exactly where real
...
|---|---|---|---|
| game length | 48 min (4×12) | 40 min (2×20) | rulepack correct; all per-game bands must be restated (§5) |
| shot clock | 24 / 14 | 30 / 20 | rulepack correct; possession-length distribution, `decide.continuation*` re-fit (§6) |
...
| foul-out | 6 of 48 min (12.5% of game per foul) | 5 of 40 min (identical 12.5%… but college fouls happen ~30% more often per minute; see §2) | `foulOutAt` correct; foul-trouble sub logic pressure is much higher in NCAA |
| 3-pt line | 23.75/22.0 ft | 22.146/21.656 ft | rulepack correct; shot EV geometry already adapts |
...
2. The efficiency gap is concentrated in FT% (−6pp), 2P% (−4pp), TOV (+20%),
...
- Shot mix: `decide.threeAppetite` (college 3PA share 37-39% vs NBA 42%: a
