...
Sourced basketball data, provenance-first. This directory exists to retire
the repo's deepest documented weakness: every acceptance band and fidelity
target was authored from memory (external review, 2026-07-26: "recollection
is not provenance"). Everything in here follows one contract:
...
schema in each shard's meta. At ~29KB/game that is compact enough that the
FULL corpus play stream fits in git; only the raw HTML stays out.
