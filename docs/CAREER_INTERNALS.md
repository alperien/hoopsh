...
| `week.ts` | One pre-NBA week: the allocation economy (weekBaseRecovery, then slots; probabilistic integer training landings at the calibrated rate), my post-game injury rolls on the franchise hazard tables (energy floor multiplies hazard), circuit game folding with grading, bracket seeding when the slate finishes, recruiting/stock/phone pulses. The training drip dies loudly: the first slot aimed at a ceilinged group logs the hidden-ceiling reveal, once per group (issue #105). |
...
| `phone-arcs.ts` | The formative arcs (issue #105): scheduled state-backed texture for the three measured dead windows — the circuit offseason (exit meeting, summer ledger, summer board, September stakes, winter block, camp countdown), the pre-combine draftPrep block (program, board spread, the league underneath, second wind, combine countdown), and the NBA draft-to-camp gap (rookie paper, summer package, the room, the camp door). Window triggers with once-ever tags; capped and cooled like every other thread, never capExempt. The exit-meeting/program/package beats carry the arc-focus assignment choice. |
