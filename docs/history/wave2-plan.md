...
> open remainder is registered in docs/REGISTER.md ("Open items — wave-2 audit").
...
## Phase A — serialized merges (orchestrator only)

After EACH merge: `npm test` + `npm run fingerprint`. Behavior-neutral branches
MUST leave the 24-seed corpus byte-identical; if one doesn't, that's a bug in the
branch and it gets fixed before the next merge — never batched.
...
## Phase B — act on the four corrections agents found in my work

These are not new features; they are defects in the project's own reference data
and gates that my earlier sessions produced and agents caught.
...
4. **Band count reads 17/17 at n=24** — confirm at n=96 with the parallel runner
   ...
   sample size, not hope.

## Phase C — Wave 2 swarm (8 agents, launched on the integrated base)

Compute discipline unchanged: agents get ≤12-game probes; **all** sweeps, band runs,
noise floors, and fingerprint re-baselines stay serialized through the orchestrator.
Note the measured hardware reality: this box is **1 physical core / 2 hyperthreads**,
so the parallel runner's ceiling here is ~1.3× — agents must not assume more.
...
## Phase D — the coordinated re-sweep (orchestrator)

One margin-objective sweep over the integrated engine (endgame flag decision
included), then: full ladder, corpus re-baseline, noise-floor regeneration, ratchet
update, push to PR #3.
