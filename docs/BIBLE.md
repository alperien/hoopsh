...
Ten agents move on a real court at 10 ticks per second: spacing, drives, kick-outs,
cuts, closeouts, help rotations, box-outs. Discrete outcomes (shots, passes, fouls,
rebounds) resolve through probability models fed by spatial context, calibrated
against author-recalled NBA ranges. The acceptance bands are still authored from
memory, not generated from sourced data; the game-flow references in
...
means, is the active roadmap arc. Games follow basketball rules and season-scale
statistics fall inside those ranges. Every point in a box score traces back to a
simulated shot at an (x, y) location. The engine is a 2D probability model with
position as an input, not a physics sim; there is no ball height. The known
simplifications are listed in full in docs/INTERNALS.md.

hoopsh is engine-first. MyPlayer careers, GM/franchise modes, historical what-ifs
("drop Jordan into 2015"), and broadcast experiences are thin apps consuming one
core's event stream. Leagues (NBA, NCAA, EuroLeague) are swappable rule packs;
rosters are human-editable data packs.

## Quickstart

Node 24+ is the only requirement. No `npm install`: the repo runs directly from
TypeScript source via Node's native type stripping and a small loader hook.
...
Optional dev tooling (`typescript`, `vitest`, `tsx`, `@types/node`) is declared
in `devDependencies`, so one plain `npm install` reproduces the full dev
environment (`npm run typecheck`, `npm run test:vitest`). Nothing at runtime
requires it; every command above works on a bare clone.
The test files import from `'vitest'`, which is not installed. `tools/hooks.mjs`
resolves that specifier to a `node:test`-backed shim (`tools/shims/vitest.ts`),
so `npm test` runs the whole suite with zero dependencies. Installing real
vitest takes over via `test:vitest`.
...
The typed event stream is the public contract. Stats, narration, and viewers are
pure consumers. Design rationale is in [ARCHITECTURE.md](./ARCHITECTURE.md).
...
All documentation is indexed in [docs/README.md](./docs/README.md): every
...
Pure physics sims are hard to calibrate; pure stat sims have no feel. hoopsh is a
hybrid spatial–stochastic sim: continuous 2D movement and decision-making, with
discrete events resolved through logistic models whose every constant lives in
one flat object, `SimParams`, the calibration surface. Player identity comes from
handcrafted attributes (what a player can do) and tendencies (what they want to
do). An elite shooter profile produces a heavy deep-three diet, off-ball gravity
that warps the defense, and star-level scoring volume as a result of these
interactions, without being scripted.

Signature mechanics:
- Shot windup: shots take 0.4–0.65s to release, so every catch-and-shoot is a
...
- Defense economics: sagging off non-shooters works because a rim-runner's open
  9-foot floater is genuinely a win for the defense.
...
`docs/INTERNALS.md`. The test suite guards determinism, possession
accounting, minutes conservation, and buzzer integrity on every change
(`npm test` prints the live count); it includes a permanent invariant suite
derived from adversarial audit rounds and an adversarial-input fixture.
Archetype tests pin player differentiation (elite shooter ≈ 25 pts on ~20 FGA
with a deep-three diet; rim-runner takes 90%+ of shots inside; non-shooting
bigs do not take low-value shots).
...
Phase 2R (current) is tuning, not building. The mechanics above are implemented
...
open work: the pass-volume arc (the probe mechanism is wired but ships staged
...
Everything here is designed so that experiences (MyPlayer careers, GM/franchise,
historical what-ifs, broadcast viewing) are thin applications built around one
engine, never entangled with it.
...
Pure physics sims are hard to calibrate; pure stat sims have no feel. hoopsh is a
hybrid spatial–stochastic sim: a continuous 2D spatial layer (movement, spacing,
defensive positioning), with discrete events (shots, passes, fouls, rebounds)
resolved probabilistically by models fed by spatial context:
...
The probability models take spatial inputs (shot distance, contest level, closing
speed, passing-lane geometry) and player ratings, and every constant lives in
`SimParams`, a single tunable parameter object. That is the calibration surface.
Realism becomes a measurable optimization problem: tune `SimParams` until league
aggregates hit acceptance bands, while archetype tests pin player differentiation.
...
2015 pace-and-space); that is what "drop Jordan into 2015" needs.
...
- Ball-handler policy scores `{shoot, drive, pass(x4), reposition}` each decision window:
  - Shot utility = predicted make prob (the same model that resolves shots; the engine
    is self-consistent) × shot value + foul-draw EV, weighed against the possession's
  ...
  - Drive utility from lane openness; help convergence spikes kick-out utility, so
...
- Game-state coupling: the scoreboard feeds back into play all game, not only in
  the endgame windows. The trailing team's defense presses up and the leader's
  sags off, through the same containment/closeout models that price every
  contest, leaned by the margin (concept 7 in the AI layer). Margins therefore
  ...
  Decided games also trigger a garbage-time concede rotation
  ...
  This rests starters about a minute per game and stops blowouts from
...
## 5. How realism is enforced

"Curry averages like Curry" is the goal. Four mechanisms enforce it:
...
2. Self-consistent AI, plus a bounded-rationality layer. The model that
   ...
   `shotMakeP`), so the EV core cannot drift from reality. On top of that core,
...
- **Current state** (B2 game-state era, measured 2026-07-28 at the
  ...
  - What ships: game-state coupling (W17), concept 7 (score
    ...
    so defense manufactures no violations). Fitted by the gain ladder
  ...
  - The landed distribution record: W14's headline gap,
    ...
    the findings).
  - Bands at the landing: 17/17 at both n=48 and n=96 on the
    acceptance base (CI-mirror `npm run batch -- --games 48` gate
    PASS; acceptance n=96 reads fga 91.3, the `4bd7a72` commit
    record), with an fga verify-base flicker: the 40×3 verify at the
    tip reads one base over the fga ceiling (swp-beta 92.60;
    alpha/gamma 17/17), and the breach moves base between adjacent
    runs (per-base fga se ≈ 0.5 at n=40, b2-scale-095 findings), so
    the per-base excursion is draw-level. The systematic content is
    the center: fga n40 sits at 91.81 (sd 0.61) against the 92.0
    ceiling at the regenerated floor (pre-B2 91.64; the coupling adds
    ~+0.4 fga on the cohort measurements, b2-trial-setC note 3), and
    the scale micro-ladder 0.85–1.0 measured no fga relief
    (b2-scale-085/090/095 findings). The headroom belongs to the next
    coordinated re-sweep (design-coupling OQ2); the staged probe is
    the known fga refund when it ships (standalone −1.0). Registered:
    REFACTOR W26. Re-measure: `npm run sweep -- --iters 0 --verify 40`.
  - Fidelity watch, star centers after the flips
    ...
    deeper; LeBron FG% 47.8% vs 50) and 2 new ones statistically on
...
- **B1 integration state** (historical, measured 2026-07-28 at the
  ...
  - Endgame management defaults ON (commit 6260cae; `endgame: false`
    ...
    below the real 4.80%; the flag closes roughly half the gap, and the
  ...
  - fga/ftPct residuals: CLOSED by a directed re-search, the repo's
    ...
    over an actual season, and widening it would certify a league less
    like basketball; the fix must move the sim, never the band. The
    excess was a possession-outcome-mix defect, not tempo: at 2.2
    possessions slower than the sourced pace 98.5, the sim
    under-produced the non-FGA possession endings (tov 12.05 vs sourced
    13.6, fta 20.3 vs 21.7), so the correct direction sheds FGA into
    turnovers/FT trips, not into pace. The re-search: 8 parallel
    strategies (multi-start jitter ×2, arithmetic seed, a 12-cell
    riskBase × ftBasePct response grid, a pace axis (blocked by a
    pace-floor break), a foul-mix axis (no reliable ftPct signal), the
    legacy objective, plus the analyst's exact n=48 pass-probability
    model built from the noise floor; riskBase ≥ −3.70 hits an
    ast/astdShare wall). Winner, the grid's robust cell (commit
    7e05c97): `pass.riskBase` −4.1869 → −3.95 and `shot.ftBasePct`
    0.69 → 0.666. Measured at the landing: 40×3 verify 17/17 on each
    of swp-alpha/beta/gamma (score 4.461; means fga 91.30, ftPct
    77.92%, tov 13.09, toward the sourced 13.6); n=96 acceptance batch
    17/17 (fga 91.6, ftPct 78.0% vs real 78.4%, tov 13.0, pf 21.0,
...
pass-volume finding above); the damping overshot, and the designed probe
...
# The hoopsh contributor covenant
...
For new code, this file is the law and `docs/PLAYBOOK.md` is the procedure:
an eight-step process, per-change-shape recipes with exemplars, STOP conditions,
and the required completion-report format. Follow it step by step.
...
- Adding, removing, or reordering any `rng` call changes every game thereafter.
  That is allowed (it's not a compatibility break) but it invalidates fingerprints;
...
### 1.5 Two time axes
- `t` / `Base.t` is game-clock time. Stops at whistles, stops at the horn.
  Minutes, pace, and all statistics key on it.
- `wallT` / `Base.wt` is the replay timeline. Advances every tick, stoppages
  included. Frames and viewers key on it.

Never mix them. Mixing them has caused two incidents: post-buzzer scoring and
...
1. Do not "tidy" SWEPT values. `shootRim: 0.485` is not a rounding error: an
   optimizer chose it against the 17 acceptance-band checks (bands.ts NBA_BANDS).
   Rounding it de-calibrates the league. If a value looks wrong, re-run the sweep
   and bake its output.
...
How to write new code for hoopsh, step by step. Audience: any agent (or
human) implementing new functionality. `AGENTS.md` is the law (what you may
and may not do); this file is the procedure (how to do it). When they
conflict, the law wins; stop and report the conflict.
...
If you later discover you need a file outside this list, STOP (Part 3) and
...
show new streams; that's expected, note it in the report).
...
- If you're unsure what a number/mechanism means and the docs don't say, state
  "the docs do not explain X" in the report; a stated unknown is preferable to
  a guess.
...
## Part 4. The dispatcher's task-briefing template

A lower-capability agent with a well-specified brief typically outperforms a
higher-capability agent with an underspecified one. Template (use verbatim):
...
game cost below. That is the price of small edges, and it is why the API
always reports the CI instead of a bare point estimate.

Measured sanity points (n = 30, seeds pinned in the tests):

- identical rosters (team vs its re-id'd clone): p̂ = 0.467,
  CI [0.302, 0.639], which contains 0.5; mean margin −0.5.
- every attribute +8 vs −8: p̂ = 0.867, CI [0.703, 0.947]; mean margin +19.8.
- equal-team margin sd ≈ 16.4 points, above the real NBA's ~13–14 for even
  matchups. Consequence for prediction: the engine maps a given true skill
  gap to a win probability closer to 50% than reality would (more game-level
  noise ⇒ flatter prob curve). Recheck this number after any calibration
  change.
...
## Cross-game state is deliberately absent
...
### What statelessness costs in prediction accuracy
...
- Injuries dominate season-total error in real backtests: a star's
  ...
  expect this to be the biggest residual.
...
The other documents state what's true; this file states the order to learn it
in, with checkpoints to verify retention.
...
Read the two contracts (~45 min):

- `packages/engine/src/core/events.ts`: every event type and its invariants;
  this is what every consumer sees.
- `packages/engine/src/sim/params.ts`: read the header primer (logit table,
  units, provenance tags), then skim the annotated defaults.
...
The guided possession trace (~60 min). Open these files side by side and
follow one possession end to end:

1. A possession begins in `sim/possession.ts#startPossession`: shot clock reset,
...
2. The clock ticks in `sim/game.ts#tick` → `tickLive`: wall clock first, then
   `movement.ts#advanceClock` (game clock; the only place `t` moves), flight
   resolution, shot-clock check, period-expiry check.
3. The handler thinks in `ai/decide.ts#decideBall`, every ~0.66s: it computes the
...
4. A pass: `passing.ts#startPass` resolves risk at launch (determinism), animates
   the flight, then `resolvePassArrival` hands the ball over and opens the
...
7. The books: `stats/box.ts` folds the event stream into exact minutes from
...
Read the emergence machinery (~30 min):
...
First-change exercises (~45 min; pick one; throwaway, do not commit):
...
- Safe: in `params.ts`, set `decide.threeAppetite: 0.5` via an override in a
  ...
  and run 10 games. Watch the 3PA share drop in the box scores; no revert needed,
  since the file itself was never touched.
- Safer: write a 10-line consumer that counts screen-adjacent pull-up threes from the
