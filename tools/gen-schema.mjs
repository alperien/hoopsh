...
export function buildTeamPackSchema() {
  ...
  return {
    ...
    description:
      ...
      `(unknown keys rejected). Two rules JSON Schema cannot express are still enforced at load ` +
      `time by 'npm run roster:validate': player ids must be unique, and starters/rotationMinutes ` +
      `must reference ids that exist on the roster.`,
    type: 'object',
