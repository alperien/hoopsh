...
//       metric/definition re-bake from the COMMITTED pbp-plays shards — the raw
//       cache is not needed (the shards are the same play streams, verbatim)
//   node tools/parse-nba.mjs --games 202511050CLE,202601200CHI --out-dir out/nba-debug
//       subset for debugging; refuses to write over the committed data/nba corpus
//
...
//                 "Defensive rebound by Team" directly after a missed TECHNICAL FT is dead-ball
//                 bookkeeping and does not close (release-audit H-06 micro)
...
//   and-one       made FG with a "Shooting foul" row within 1s of game clock, in the SHOOTER's
//                 column (bbref prints fouls in the fouled team's column) and naming the shooter
//                 as the drawn-by player — side/player checks kill cross-possession false
//                 positives inside the window (release-audit L-54)
//   second chance possession contains >= 1 LIVE offensive rebound; "Offensive rebound by Team"
//                 rows that are dead-ball scorer bookkeeping (after missed non-final FTs, after
//                 missed technical FTs, at the period horn) do not mark (release-audit H-06)
...
// Flag parsing lives in main() (bottom of file) so the pure metric functions
// above it stay importable by tests and by tools/redteam-probes/ without
// side effects — and so a malformed flag on the RUNNER's argv cannot detonate
// tools/args.mjs's loud parser at import time.
...
export function extractPlays(html, id) {
...
export function extractGameMeta(html, id) {
...
// plus one ADVISORY check (never fails a game):
//  3. clock sanity: within one period the clock only counts down, and never
//     exceeds the period length. bbref stamps the odd between-period
//     substitution row at 12:00.0 of the OLD period (3 rows in the committed
//     corpus, all subs); those rows carry no metric weight, so they are
//     counted in `clockJumps` and warned, not rejected (release-audit L-55).
export function validateGame(plays, boxFinal) {
  const last = plays.at(-1);
  const scoreboardFinal = [last.a, last.h];
  let textA = 0, textH = 0, mismatches = 0, clockJumps = 0;
  let prevA = 0, prevH = 0;
  let prevRow = null;
  for (const p of plays) {
    if (p.clockSec > (p.q <= 4 ? 720 : 300)
      || (prevRow && prevRow.q === p.q && p.clockSec > prevRow.clockSec)) clockJumps++;
    prevRow = p;
  ...
  return { ok, scoreboardFinal, textFinal: [textA, textH], boxFinal, mismatches, clockJumps };
}

// ---------------------------------------------------------------- flow metrics (mirrors harness/src/flow.ts)
export function flowMetrics(plays) {
...
// A genuine and-one foul row sits in the SHOOTER's column (bbref prints foul
// rows in the fouled team's column — 862/863 windowed shooting fouls in the
// corpus) and names the shooter as the drawn-by player. Without both checks,
// an unrelated shooting foul on the NEXT possession that lands inside the 1s
// window marks a cross-possession and-one — 4 rows in the committed corpus,
// e.g. a make at one end followed by an instant take foul at the other
// (release-audit L-54).
const isAndOneFoulRow = (row, madeShot) => {
  if (row.side !== madeShot.side || !/Shooting foul/i.test(row.text)) return false;
  const shooter = madeShot.text.slice(0, madeShot.text.indexOf(' makes '));
  return row.text.includes(`(drawn by ${shooter})`);
};

/** true when the made FG at plays[i] (game clock e0) draws a shooting foul within 1s. */
const hasAndOneWithin1s = (plays, i, e0) => {
  for (let j = i + 1; j < plays.length && elapsed(plays[j]) - e0 <= 1; j++) {
    if (isAndOneFoulRow(plays[j], plays[i])) return true;
  }
  return false;
};

export function grammarMetrics(plays) {
  ...
  for (let i = 0; i < plays.length; i++) {
    ...
    if (isMadeFG(p.text) && hasAndOneWithin1s(plays, i, e0)) andOnes++;
  }
...
//   defensive rebound (player or team; EXCEPT the dead-ball team-rebound row after a missed
//   technical FT — the pre-foul possession resumes, nothing changed hands), turnover, made
//   final FT of a plain "N of N" trip (technical/flagrant/clear-path FTs never match and
//   correctly do not end possessions), period end. Lengths are boundary-to-boundary within a
//   period; team OREBs that are dead-ball scorer bookkeeping (see deadBallTeamOreb below) do
//   NOT mark a possession as second-chance.
export function possessionMetrics(plays) {
  const lens = [];
  let secondChance = 0;
  let curStart = 0, curHadOreb = false, curQ = 1;

  // "Offensive rebound by Team" rows that are dead-ball scorer bookkeeping,
  // not live board work — the ball was never up for grabs, so they must not
  // mark the possession second-chance (release-audit H-06: 362 such rows
  // inflated the shipped share ~8%):
  //  1. after a missed NON-FINAL numbered FT ("misses free throw 1 of 2"):
  //     nobody may play the ball between attempts;
  //  2. after a missed TECHNICAL FT: the ball returns to the pre-foul
  //     possession regardless of the board;
  //  3. at the period horn (clock < 1s, incl. after desperation heaves and
  //     "Team misses heave shot" rows): bbref books the unsecured miss to the
  //     shooting team as the period dies. Detected as nothing but
  //     dead-ball rows (subs, timeouts, replay reviews, the End-of-period
  //     row) between the rebound and the period roll — a rebound at 0:00.x
  //     followed by live action (a putback try, a foul's FTs) stays counted.
  const deadBallTeamOreb = (i) => {
    const prevText = plays[i - 1]?.text ?? '';
    const ft = prevText.match(/misses .*free throw (\d) of (\d)/);
    if (ft && ft[1] !== ft[2]) return true;
    if (/misses technical free throw/i.test(prevText)) return true;
    if (plays[i].clockSec === 0) {
      for (let j = i + 1; j < plays.length && plays[j].q === plays[i].q; j++) {
        if (!/enters the game|timeout|Instant Replay|^End of /i.test(plays[j].text)) return false;
      }
      return true;
    }
    return false;
  ...
  for (let i = 0; i < plays.length; i++) {
    ...
    if (/Offensive rebound by/i.test(t)) {
      if (!(/by Team/i.test(t) && deadBallTeamOreb(i))) curHadOreb = true;
      continue;
    }
    if (/Defensive rebound by/i.test(t)) {
      // dead-ball twin of deadBallTeamOreb case 2: "Defensive rebound by
      // Team" directly after a missed technical FT is bookkeeping — play
      // resumes with the pre-foul possession untouched (7 corpus rows
      // falsely closed here; release-audit H-06 micro)
      const prevText = plays[i - 1]?.text ?? '';
      if (!(/by Team/i.test(t) && /misses technical free throw/i.test(prevText))) close(e);
      continue;
    }
    if (isTurnover(t)) { close(e); continue; }
    if (isMadeFG(t)) {
      if (!hasAndOneWithin1s(plays, i, e)) close(e);
      continue;
    }
...
function main() {
  ...
  const fromShards = argv.includes('--from-shards');

  // --games is a debugging subset: writing it over data/nba would silently
  // replace the committed 184-game corpus with the subset's aggregates (the
  // shard writer would even DELETE the other games' month files' content) —
  // refuse unless the author aims it somewhere scratch (release-audit L-56).
  if (onlyGames && outDir === 'data/nba') {
    console.error('parse-nba: --games writes a SUBSET corpus — pass --out-dir out/nba-subset (or any scratch path) so the committed data/nba corpus is not overwritten');
    process.exitCode = 2;
    return;
  }
  if (onlyGames && writeReference) {
    console.error('parse-nba: --write-reference with --games would bake a subset into flow-reference.json (its values claim the full corpus) — drop one of the two flags');
    process.exitCode = 2;
    return;
  }
  ...
  if (fromShards) {
    // Metric/definition re-bake: the committed monthly shards ARE the full
    // play streams, so corpus + reference regenerate without the gitignored
    // raw HTML cache (release-audit H-06 shipped exactly this way).
    const shardsDir = 'data/nba/pbp-plays';
    const shardFiles = existsSync(shardsDir)
      ? readdirSync(shardsDir).filter((f) => /^plays-\d{4}-\d{2}\.json$/.test(f)).sort()
      : [];
    if (shardFiles.length === 0) {
      console.error(`parse-nba: --from-shards found no committed shards in ${shardsDir}`);
      process.exitCode = 1;
      return;
    }
    const rows = [];
    for (const f of shardFiles) {
      const shard = JSON.parse(readFileSync(join(shardsDir, f), 'utf8'));
      for (const [id, g] of Object.entries(shard.games)) {
        if (onlyGames && !onlyGames.includes(id)) continue;
        rows.push([id, g]);
      }
    }
    rows.sort((a, b) => (a[0] < b[0] ? -1 : 1)); // same order as the cache path's sorted game ids
    for (const [id, g] of rows) {
      try {
        // inverse of the shard writer's tuple rows: [q, clockSec, side(a/h/null), text, a, h]
        const plays = g.plays.map(([q, clockSec, side, text, a, h]) =>
          ({ q, clockSec, side: side === 'a' ? 'away' : side === 'h' ? 'home' : null, text, a, h }));
        const meta = {
          away: g.away, home: g.home, boxFinal: [g.final[0], g.final[1]],
    ...
    if (gameFiles.length === 0) {
      console.error(`parse-nba: no cached games in ${cacheDir} — run npm run nba:fetch first (or re-bake the committed corpus with --from-shards)`);
      process.exitCode = 1;
      return;
    }
    for (const id of gameFiles) {
      try {
        ...
        build(id, extractGameMeta(html, id), extractPlays(html, id));
      } catch (err) {
        failures.push(id);
        games.push({ id, error: String(err.message ?? err) });
      }
  ...
  // advisory clock-sanity tally (validateGame check 3 — see its header)
  const clockJumpRows = good.reduce((s, g) => s + g.validation.clockJumps, 0);
  if (clockJumpRows > 0) {
    console.log(`  WARN clock-sanity: ${clockJumpRows} row(s) run against the period clock (bbref stamps between-period subs at 12:00.0 of the OLD period) — advisory; the rows carry no metric weight`);
  }
  // A committed-shards re-bake must be lossless: every committed game already
  // validated once, so ANY failure here is a parser regression — abort before
  // the write phase can silently drop games from the corpus it regenerates.
  if (fromShards && outDir === 'data/nba' && good.length !== games.length) {
    console.error(`parse-nba: --from-shards re-bake would drop ${games.length - good.length} committed game(s) — parser regression, nothing written`);
    process.exitCode = 1;
    return;
  }
  ...
  const corpus = {
    meta: {
      ...
      generatedAt: today(),
      pipeline: 'npm run nba:fetch -- --season 2025-26 && npm run nba:parse -- --write-reference; metric/definition re-bakes from the committed shards: npm run nba:parse -- --from-shards --write-reference',
      games: good.length,
      failed: failures,
      dates: Object.fromEntries(Object.entries(good.reduce((m, g) => { m[g.date] = (m[g.date] ?? 0) + 1; return m; }, {})).sort()),
      validation: {
        pass: good.length, fail: failures.length, clockJumpRows,
        checks: 'per game: (1) score column is monotonic and one-sided per event, (2) every scoring event’s delta equals the points its text implies, (3) text-summed points == scoreboard final == scorebox final for both teams. Advisory (counted in clockJumpRows, never fails a game): rows whose clock runs backwards within a period or exceeds the period length — bbref stamps the odd between-period substitution at 12:00.0 of the OLD period.'
      },
      playsFiles,
  ...
  if (writeReference) {
    ...
    const grammar = {
      ...
      andOnesPerGame: { value: d.grammar.andOnesPerGame.mean, range: [d.grammar.andOnesPerGame.min, d.grammar.andOnesPerGame.max], dist: gDist(d.grammar.andOnesPerGame), grade: 'A', basis: `corpus n=${good.length}, both teams: made FG with a shooting-foul row within 1s of game clock, same column as the make and naming the shooter as drawn-by (meta.definitions.andOne)` },
      secondChanceShareOfPoss: { value: d.possessions.secondChanceShare.pooled, range: [d.possessions.secondChanceShare.min, d.possessions.secondChanceShare.max], dist: { ...strip(d.possessions.secondChanceShare), unit: 'per game share; value = pooled' }, grade: 'A', basis: `corpus: share of ALL possessions (both teams pooled) containing >= 1 LIVE offensive rebound — dead-ball team-rebound bookkeeping never marks (missed non-final FTs, missed technical FTs, period-horn rows; meta.definitions.secondChance, release-audit H-06). NOTE for flow.ts: its report divides both-team second-chance possessions by poss/2 (a per-team denominator), which reads ~2x this definition — reconcile before gating.` }
    };
    ...
    const ref = {
      meta: {
        ...
        definitions: {
          ...
          possession: 'boundaries = made FG (and-1 trips end at the made final FT), defensive rebound (player or team — except the dead-ball "by Team" row directly after a missed technical FT, which closes nothing because the pre-foul possession resumes), turnover, made final FT of a plain N-of-N trip, period end; length = game-clock seconds between boundaries (FT sequences freeze the clock); technical/flagrant/clear-path FTs do not end possessions',
          ...
          andOne: 'made FG with a shooting-foul row within 1s of the same game clock, in the shooter\'s column (bbref prints foul rows in the fouled team\'s column) and naming the shooter as the drawn-by player — without the side/player checks, unrelated shooting fouls inside the window read as cross-possession and-ones',
          secondChance: 'possession contains >= 1 LIVE offensive rebound; "Offensive rebound by Team" rows that are dead-ball scorer bookkeeping never mark: after a missed non-final numbered FT, after a missed technical FT, and at the period horn (clock < 1s with nothing but subs/timeouts/replay/End-of-period rows before the period rolls, incl. after desperation heaves)',
          ambiguitiesResolved: [
            'OREB denominator: bbref logs "Offensive rebound by Team" bookkeeping rows (dead-ball after missed non-final FTs and missed technical FTs, unsecured misses at the period horn) — excluded from the putback denominator and from second-chance marking; the sim has no team-rebound events so player rebounds are the comparable base. Anchor-definition values are preserved in dist.legacyAnchorDefinition. The shipped corpus originally excluded only the missed-non-final-FT class, inflating secondChanceShareOfPoss ~8% (0.132 -> 0.122 re-baked); release-audit H-06.',
